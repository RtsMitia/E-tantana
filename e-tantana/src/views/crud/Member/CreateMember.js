...
export default class CreateMember extends AdminLayout {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getMember = (index) => {
    this.setState({
      ...
      birthdate:
        ...
        this.state.members[index].birthdate.split("T")[0],
      ...
      entryDate:
        ...
        this.state.members[index].entry_date.split("T")[0],
      promiseDate:
        this.state.members[index].promise_date &&
        this.state.members[index].promise_date.split("T")[0],
  ...
  getMembers = (page=this.state.page, pageNumber=this.state.pageNumber) => {
    ...
    fetch(
      api(`members?page=${Number(page)}&pageNumber=${Number(pageNumber)}`)
    ).then((res) => {
      if (res.ok)
        res.json().then((data) => {
          if (data && data.data)
          ...
          <CCol xs={12}>
            <CCard className="mb-4">
              ...
              <CRow className="mb-3">
                ...
                <CCol sm={3}>
                  <CFormLabel htmlFor="promiseDate">Date velirano *</CFormLabel>
                  <CFormInput
                    type="date"
                    id="promiseDate"
