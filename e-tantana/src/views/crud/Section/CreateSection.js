...
export default class CreateSection extends AdminLayout {
  constructor(props) {
    super(props);
    this.state = {
      list: [],
    ...
    if (
      ...
      const option = {
        ...
        body: JSON.stringify({
          ...
          id: this.state.list[this.state.index].id,
        }),
      };
  ...
  insert = (event) => {
    if (
      ...
      fetch(api("sections"), option).then((res) => {
        this.setPending(false);
        if (res.ok) {
          res.json().then((data) => {
        ...
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader color="green">
              <strong> Ajout de section </strong>
